...
def J_res(model, col, domain):
    ...
    return 1/N_col*torch.norm(residual, p = 2)**2
...
def J_obs(model, obs):
    ...
    predictions = model(obs.data[:, 0].view(-1, 1))
    observations = obs.data[:, 1].view(-1, 1)
    
    return 1/obs.N_obs*torch.norm(predictions - observations, p = 2)**2
